...
const api = {
  GetState: () => window.api.getState(),
  GetWindowState: () => window.api.getWindowState(),
  SwitchToUrl: (url) => window.api.switchToUrl(url),
  DeleteUrl: (url) => window.api.deleteUrl(url),
  TogglePin: (url) => window.api.togglePin(url),
  ToggleRotation: (url) => window.api.toggleRotation(url),
  TogglePreload: (url) => window.api.togglePreload(url),
  AdjustZoom: (delta) => window.api.adjustZoom(delta),
  SetAlias: (url, alias) => window.api.setAlias(url, alias),
  ToggleGridMode: () => window.api.toggleGridMode(),
  MoveUrl: (url, delta) => window.api.moveUrl(url, delta),
  AddTrustedHost: (host) => window.api.addTrustedHost(host),
  RemoveTrustedHost: (host) => window.api.removeTrustedHost(host),
  ToggleScrollLock: () => window.api.toggleScrollLock(),
...
async function handleAdd() {
  console.log('[JS] handleAdd fired. url input=', JSON.stringify(addUrlInput.value))
  try {
    const raw = addUrlInput.value.trim()
    if (!raw) { console.warn('[JS] empty url, aborting'); return }
    ...
    console.error('[JS] handleAdd failed:', e)
